...
4. **Cloud Run Container Deployment** *(Lifecycle: Runtime)*
   - Single-port container deployment where Stage 3 Python MCP server listens on Cloud Run's `$PORT` (8080), intercepts `malloy_getContext`, and relays execution queries (`malloy_executeQuery`, `malloy_compile`, `tools/list`) to the official Node.js `@malloy-publisher/server` running on internal port 5050.
...
| **Runtime** | **Requirement 3:**<br>Agent Integration & MCP Dual-Pathway Execution | [`step_3_mcp_server/`](file:///home/maxantipev/analytics-malloy-publisher/C_Context/step_3_mcp_server)<br>└── `step_3_mcp_dual_pathway_server.py` | Hosts public MCP endpoint on port 8080. Implements **Path A** (<0.05ms in-RAM Fast Router) and **Path B** (ChromaDB vector search), synthesizing the 3-section context for `malloy_getContext`. Relays execution to Node on 5050. |
| **Runtime** | **Requirement 4:**<br>Cloud Run Container Deployment | [`deploy/`](file:///home/maxantipev/analytics-malloy-publisher/C_Context/deploy)<br>└── `entrypoint.sh` | Cloud Run multi-process supervisor: launches Node execution engine on 5050, health-checks it, starts Python MCP server on 8080, and traps `SIGTERM`/`SIGINT` for clean teardown. |
